...
const lessonController = {
    ...
    async postCreateLesson(req, res) {
        ...
        try {
            ...
            res.redirect(`/courses/${slug}/lessons/dashboard`);
        } catch (error) {
            console.error(error);
            const course = await Course.getBySlug(slug);
            const lessons = await Lesson.getAllByCourseId(course.id);
    ...
    async getEditLesson(req, res) {
        try {
            ...
            if (!lesson) return res.redirect(`/courses/${slug}/lessons/dashboard`);
    ...
    async postRestoreLesson(req, res) {
        try {
            ...
            res.redirect(`/courses/${slug}/lessons/dashboard/rebin`);
        } catch (error) {
            console.error(error);
            res.redirect(`/courses/${req.params.slug}/lessons/dashboard/rebin`);
        }
    },
